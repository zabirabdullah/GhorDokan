    ...
    <div className="min-h-screen bg-gray-50 text-gray-900">
      ...
      <main className="mx-auto max-w-7xl px-6 pb-12 pt-8">
        ...
        {showConfirmModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
            <div className="mx-4 w-full max-w-md rounded-xl bg-white p-6 shadow-2xl">
              <h4 className="mb-4 text-xl font-bold text-gray-900">Confirm Status Change</h4>
        ...
        <div className="mb-8 rounded-lg border border-gray-200 bg-white p-6 shadow">
          <h2 className="mb-4 text-2xl font-bold text-gray-900">Order Details</h2>
          <div className="grid gap-4 sm:grid-cols-3">
            ...
            <div>
              ...
              <p className="text-lg font-semibold text-gray-900">Ahmed Khan</p>
            </div>
            <div>
              <p className="text-sm text-gray-600">Total Price</p>
              <p className="text-lg font-semibold text-amber-600">₹2,500</p>
            </div>
          </div>
        </div>
        ...
        {/* Customer Details */}
        <div className="mb-8 rounded-lg border border-gray-200 bg-white p-6 shadow">
          <h3 className="mb-4 text-xl font-bold text-gray-900">Customer Details</h3>
        ...
        {/* Comments Section */}
        <div className="mb-8 rounded-lg border border-gray-200 bg-white p-6 shadow">
          <h3 className="mb-4 text-xl font-bold text-gray-900">Comments</h3>
        ...
        {/* Products Section */}
        <div className="mb-8 rounded-lg border border-gray-200 bg-white p-6 shadow">
          <h3 className="mb-4 text-xl font-bold text-gray-900">Products</h3>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-200 bg-gray-50">
                  <th className="px-4 py-3 text-left text-sm font-semibold text-gray-900">Product ID</th>
              ...
              <tbody>
                {products.map((product, index) => (
                  <tr key={product.id} className={`border-b border-gray-200 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
                    <td className="px-4 py-3 text-sm text-gray-900 font-medium">{product.id}</td>
                    <td className="px-4 py-3 text-sm text-gray-600">{product.name}</td>
                    <td className="px-4 py-3 text-sm text-gray-600">{product.amount}</td>
                    <td className="px-4 py-3 text-sm text-gray-600">{product.price}</td>
                    <td className="px-4 py-3 text-sm text-gray-600">{product.category}</td>
                    <td className="px-4 py-3 text-sm font-semibold text-amber-600">{product.subtotal}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-6 shadow">
          <h3 className="mb-4 text-xl font-bold text-gray-900">Order Status</h3>
          <div className="grid gap-3 sm:grid-cols-4 lg:grid-cols-7">
            {statuses.map((status) => (
              <button
  key={status.key}
  onClick={() => handleStatusClick(status.key)}
  className={`relative overflow-hidden rounded-xl px-4 py-3 text-sm font-bold transition-all duration-300 border-2 ${
